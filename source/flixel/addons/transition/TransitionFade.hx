...
class TransitionFade extends TransitionEffect
{
	...
	override public function update(elapsed:Float):Void
	{
		...
		//@:privateAccess back.setGraphicSize(Math.round(cameras[0].viewWidth), Math.round(cameras[0].viewHeight));
		//back.updateHitbox();
	}
	...
	function makeSprite(DirX:Float, DirY:Float, region:FlxRect):FlxSprite
	{
		...
		{
			// no direction
			// TODO: Optimize this by using 1x1
			s.makeGraphic(Std.int(region.width), Std.int(region.height), _data.color, true, "transition");
		}
		else if (DirX == 0 && Math.abs(DirY) > 0)
		{
			// vertical wipe
			...
			angle = DirY > 0 ? 90 : 270;
			s.makeGraphic(1, Std.int(region.height * 2), _data.color, true, "transition");
			pixels = s.pixels;
		...
		else if (Math.abs(DirX) > 0 && DirY == 0)
		{
			...
			angle = DirX > 0 ? 0 : 180;
			s.makeGraphic(Std.int(region.width * 2), 1, _data.color, true, "transition");
			pixels = s.pixels;
