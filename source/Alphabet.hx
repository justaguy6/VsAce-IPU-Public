...
class Alphabet extends FlxSpriteGroup
{
	...
	override function update(elapsed:Float)
	{
		if (isMenuItem)
		{
			...
			var lerpVal = 0.30; //  CoolUtil.boundTo(elapsed * 9.6, 0, 1);
			y = FlxMath.lerp(y, (scaledY * 120) + (FlxG.height * 0.48) + yAdd, lerpVal);
		...
		if (isOption)
		{
			...
			var lerpVal = 0.30; //  CoolUtil.boundTo(elapsed * 9.6, 0, 1);
			y = FlxMath.lerp(y, (scaledY * 70) + (FlxG.height * 0.48) + yAdd, lerpVal);
			if(forceX != Math.NEGATIVE_INFINITY) {
				x = forceX;
			} else {
				x = FlxMath.lerp(x, (targetY * 20) + 90 + xAdd, lerpVal);
